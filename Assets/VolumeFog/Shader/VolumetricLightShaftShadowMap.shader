Shader "Unlit/VolumetricLightShaftShadowMap"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float map(float3 p){
                ...
                float4 pos = mul(unity_WorldToShadow[0], float4(p,1));
                float2 uv = saturate(pos.xy  / _Size * 0.5 + 0.5);
                float shadow = SAMPLE_DEPTH_TEXTURE_LOD(_ShadowMapTexture, float4(pos.xy,0,2));
				// float shadow = UNITY_SAMPLE_SHADOW(_ShadowMapTexture, pos.xyz);
				// shadow = _LightShadowData.r + shadow * (1-_LightShadowData.r);
				// float z = 1 - (pos.z - _Near) / (_Far - _Near);
                shadow = shadow < pos.z ? 0.0 : 1.0;
				return _Intensity * shadow;
			}
            ...
            fixed4 frag (v2f vert, fixed facing : VFACE) : SV_Target
            {
                ...
                float4 pp = mul(unity_WorldToShadow[0], vert.worldPos);
				// float shadow = UNITY_SAMPLE_SHADOW(_ShadowMapTexture, pp.xyz);
				// shadow = _LightShadowData.r + shadow * (1-_LightShadowData.r);
				// return shadow;
				// return UNITY_SAMPLE_SHADOW(_ShadowMapTexture, vert.worldPos.xyz  / 1000.0);
				return tex2D(_ShadowMapTexture, frac(vert.worldPos.xz));
            }
            ENDCG
        }
    }
